...
//Hago operacion de interseccion guardando todo en una lista nueva, DESPUES HAGO OPERACION DE VOLCADO DE ESA LISTA NUEVA EN EL SET DONDE SE ASIGNE,
//EN el que se asigne es como al hacer copia, el asignado se borra todo y se le meto lo nuevo
public static String aplicarOperacion(String set, String set2, String set_a_Volcar,String tipoOperacion){
    String elem1="";
    String elem2="";
    String setUnion = Generator.nuevaTemp();
    int contador=0;
    boolean tipoChar=false;
    char aux;
    int au;

    if(getTipoSet(set).equals("CHAR")){
        tipoChar=true;
    }

    if(tipoOperacion.equals("UNION")){

        PLXC.out.println("# Aplicamos UNION");
        
        LinkedList<String> listaSet1 = dictSets.get(set);
        LinkedList<String> listaSet2 = dictSets.get(set2);
        // PLXC.out.println(listaSet1.toString());
        // PLXC.out.println(listaSet2.toString());

        LinkedList<String> listaUnion = new LinkedList<>();
        listaUnion.add(0, listaSet1.get(0)); //Asigno ya al conjunto resultado el tipo
        dictSets.put(setUnion,listaUnion);

        int tam1=listaSet1.size();
        int tam2=listaSet2.size();  // [int,1,2,3]  tam: 4

        for(int i=1 ; i<tam1 ; i++){
            elem1=listaSet1.get(i);
            
            if(!listaUnion.contains(elem1)){
                listaUnion.add(elem1);
                if(tipoChar){
                    elem1=elem1.replaceAll("'","");
                    aux= elem1.charAt(0);
                    Generator.salida(Generator.ARRAY, String.valueOf(contador), String.valueOf(aux), setUnion);
                }else{
                    Generator.salida(Generator.ARRAY, String.valueOf(contador), elem1, setUnion);
                }
                contador++;
            }
        }
        ...
        for(int i=1 ; i<tam2 ; i++){
            elem2=listaSet2.get(i);
            if(!listaUnion.contains(elem2)){
                listaUnion.add(elem2);
                if(tipoChar){
                    elem2=elem2.replaceAll("'","");
                    aux= elem2.charAt(0);
                    Generator.salida(Generator.ARRAY, String.valueOf(contador), String.valueOf(aux), setUnion);
                }else{
                    Generator.salida(Generator.ARRAY, String.valueOf(contador), elem2, setUnion);
                }
        ...
        // PLXC.out.println("LISTA RESULTADO FUNCION APLICAR OPERACION: "+listaUnion.toString());
        // PLXC.out.println("SET UNION NUEVO: "+setUnion);
        dictSets.put(setUnion, listaUnion);
        TablaSimbolos.VolcarArrayAux_a_Original_SET(set_a_Volcar, setUnion,true);

     

    }else if(tipoOperacion.equals("RESTA")){

        PLXC.out.println("#Aplicamos RESTA");
        
        LinkedList<String> listaSet1 = dictSets.get(set);
        LinkedList<String> listaSet2 = dictSets.get(set2);

        LinkedList<String> listaResul = new LinkedList<>();
        listaResul.add(0, listaSet1.get(0)); //Asigno ya al conjunto resultado el tipo
        dictSets.put(setUnion,listaResul);
        ...
        for(int i=1 ; i<tam1 ; i++){
            ...
            if(!listaSet2.contains(elem1)){
                listaResul.add(elem1);
                Generator.salida(Generator.ARRAY, String.valueOf(contador), elem1, setUnion);
                contador++;
            }
        }
        ...
        // PLXC.out.println("LISTA RESULTADO FUNCION APLICAR OPERACION: "+listaResul.toString());
        // PLXC.out.println("SET UNION NUEVO: "+setUnion);
        dictSets.put(setUnion, listaResul);
        TablaSimbolos.VolcarArrayAux_a_Original_SET(set_a_Volcar, setUnion,true);

    }else{
        PLXC.out.println("#Aplicamos INTERSECCION");
        LinkedList<String> listaSet1 = dictSets.get(set);
        LinkedList<String> listaSet2 = dictSets.get(set2);
...
public static String operatoriaInterElementoSet(String set, String elem, String array_aux, int contador){
    // dictSets.put(array_aux,null);
    LinkedList<String> lista = dictSets.get(set);
    LinkedList<String> lista2 = dictSets.get(array_aux);

   
    if(lista.contains(elem)){
       lista2.add(elem);
        Generator.salida(Generator.ARRAY, String.valueOf(contador), elem, array_aux);
    }
...
// PLXC.out.println("-DENTRO 0");
    // PLXC.out.println("ARRAY ORIGINAL: "+arrayOriginal+ " ARRAY A VOLCAR: "+ arrayAux);

    if(!arrayOriginal.equals(arrayAux)){ 
                // PLXC.out.println("-DENTRO 1");
